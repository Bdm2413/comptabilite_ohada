...
try {
    ...
    $sql = "
        ...
        WHERE pc.actif = 'Oui' AND pc.tableau = 'Résultat' AND pc.societe_id = ?
...
<body class="bg-slate-900 text-slate-100">
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700 mb-6">
                <form method="GET">
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <button type="button" id="btnToggleCondense" onclick="toggleCondensedMode()" class="px-6 py-2 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2">
                        ...
                        <button type="button" onclick="exportExcelAvecComptes()" class="px-6 py-2 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2" title="Exporter avec le détail des comptes par rubrique">
                ...
                <div class="overflow-x-auto overflow-y-auto max-h-[700px]">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-700">
                            ...
                            ?>
                                <tr class="hover:bg-slate-700/30 transition-colors <?= $is_solde ? 'bg-slate-700/50 font-bold' : '' ?>" data-type="<?= $info['type'] ?>">
    ...
    <script>
        let isCondensed = localStorage.getItem('crCondensed') === 'true';

        function toggleCondensedMode() {
            isCondensed = !isCondensed;
            applyCondensedMode();
            localStorage.setItem('crCondensed', isCondensed);
        }
        ...
        function applyCondensedMode() {
            ...
            document.querySelectorAll('tr[data-type]').forEach(row => {
                ...
                row.style.display = (isCondensed && type !== 'solde') ? 'none' : '';
            });

            // En mode condensé, refermer tous les détails ouverts
            if (isCondensed) {
            ...
            btn.textContent = isCondensed ? 'Détaillé' : 'Condensé';
            if (icon) icon.className = isCondensed ? 'fas fa-expand-alt' : 'fas fa-compress-alt';
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            if (isCondensed) applyCondensedMode();
        });
    </script>
    <script>
        // Filtre balance : n'afficher que les comptes de Résultat (classes 6, 7, 8)
        window.balanceTableauFilter = 'Resultat';
    </script>
