...
$lettrage_filter = $_GET['lettrage'] ?? 'tous';
...
$sql = "
    SELECT
        ...
        le.debit, le.credit,
        COALESCE(pt_ligne.abreviation, pt_ecriture.abreviation, pt_ligne.nom, pt_ecriture.nom) as tiers_nom,
        NULL as lettrage,
    ...
    WHERE le.compte = ? AND e.societe_id = ? AND e.statut = 'Validé' AND e.date_ecriture BETWEEN ? AND ?
";
...
if ($lettrage_filter === 'lettrees') {
    $sql .= " AND le.statut_lettrage = 'Lettré'";
} elseif ($lettrage_filter === 'non_lettrees') {
    $sql .= " AND (le.statut_lettrage != 'Lettré' OR le.statut_lettrage IS NULL)";
}
...
$headers = ['Date', 'N° Écriture', 'Journal', 'Compte', 'N° Pièce', 'N° Facture', 'Libellé', 'Tiers', 'Lettrage', 'Débit', 'Crédit', 'Solde'];
$col = 'A';
...
$sheet->setCellValue('L' . $row, $solde_initial);
$sheet->getStyle('L' . $row)->getNumberFormat()->setFormatCode('#,##0.00');
$sheet->getStyle('A' . $row . ':L' . $row)->getFont()->setBold(true);
$sheet->getStyle('A' . $row . ':L' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FF1E293B');
$sheet->getStyle('A' . $row . ':L' . $row)->getFont()->getColor()->setARGB('FFFFFFFF');
$row++;
...
foreach ($lignes as $ligne) {
    ...
    if (!empty($ligne['lettrage'])) {
        $lettrage_display = $ligne['lettrage'];
        if ($ligne['statut_lettrage'] === 'Lettré') {
            $lettrage_display .= ' ✓';
        }
    }
    $sheet->setCellValue('I' . $row, $lettrage_display);

    $sheet->setCellValue('J' . $row, $ligne['debit'] > 0 ? $ligne['debit'] : '');
    $sheet->setCellValue('K' . $row, $ligne['credit'] > 0 ? $ligne['credit'] : '');
    $sheet->setCellValue('L' . $row, $solde_courant);
    ...
    if ($ligne['debit'] > 0) {
        $sheet->getStyle('J' . $row)->getFont()->getColor()->setARGB('FF00AA00');
    }
    if ($ligne['credit'] > 0) {
        $sheet->getStyle('K' . $row)->getFont()->getColor()->setARGB('FFFF0000');
    }

    // Mettre en vert le lettrage si complet
    if ($ligne['statut_lettrage'] === 'Lettré') {
...
$sheet->getStyle('K' . $row)->getNumberFormat()->setFormatCode('#,##0.00');
$sheet->getStyle('A' . $row . ':L' . $row)->getFont()->setBold(true);
$sheet->getStyle('A' . $row . ':L' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FF475569');
$sheet->getStyle('A' . $row . ':L' . $row)->getFont()->getColor()->setARGB('FFFFFFFF');
$row++;
