...
try {
    ...
    // CF (Réserves indisponibles) en N = Compte 11
    $sql_reserves_n = "
    ...
    $stmt_reserves_n->execute([$date_fin, $societe_id, $societe_id]);
    ...
    //
    // IMPORTANT:
    // Cette approche est utilisée en l'absence de décision d'affectation du résultat
    // par l'Assemblée Générale. Normalement, le résultat devrait être affecté en:
    // - Réserves (obligatoires ou libres)
    // - Dividendes (distribution)
    // - Report à nouveau (solde non affecté)
    //
    // EXEMPLE CONCRET:
    // Si Résultat 2024 = -1,586,846,772.89 et Résultat 2025 = -585,109,602.56
    // Alors en 2026: CH = -1,586,846,772.89 + (-585,109,602.56) = -2,171,956,375.45
    // Si Résultat 2026 = +2,000,000,000.00
    // Alors en 2027: CH = -2,171,956,375.45 + 2,000,000,000.00 = -171,956,375.45
    // ═══════════════════════════════════════════════════════════════════════════════

    // Compte 12: Report à nouveau comptable proprement dit (écritures d'affectation)
    $sql_report_12 = "
    ...
    $stmt_report_12->execute([$date_fin, $societe_id, $societe_id]);
    $report_12 = $stmt_report_12->fetchColumn();

    // Compte 13: CUMUL de tous les résultats antérieurs (jusqu'à fin N-1)
    // Cette requête récupère AUTOMATIQUEMENT tous les résultats cumulés:
    // - date_ecriture < $date_debut signifie "tout avant le début de l'exercice N"
    // - Donc en 2026, cela prendra les résultats 2024 + 2025 automatiquement
    $sql_resultat_n1 = "
    ...
    $stmt_resultat_n1->execute([$date_debut, $societe_id, $societe_id]);
    $resultat_n1 = $stmt_resultat_n1->fetchColumn();

    // CH = Compte 12 (affectations manuelles) + Compte 13 (cumul automatique des résultats)
    ...
    // PRINCIPE SYSCOHADA : le bilan est une photo cumulative jusqu'à date_fin.
    // On utilise donc <= date_fin (et NON BETWEEN date_debut AND date_fin) pour rester
    // cohérent avec le calcul cumulatif de tous les autres postes de bilan (actif et passif).
    ...
    $stmt_resultat->execute([$date_fin, $societe_id, $societe_id]);
    ...
    foreach ($comptes_n1 as $compte) {
        ...
        elseif ($compte['bd'] != $compte['bc']) {
            if ($solde > 0 && !empty($compte['bd'])) {
                ...
                $passif_n1[$ref]['net'] += abs($solde); // BD≠BC: solde créditeur toujours négatif au passif
    ...
    // PASSIF N-1
    // Calcul spécifique pour CF, CH et CJ en N-1
    // (Même logique cumulative que pour N, mais pour la période N-1)

    // CF (Réserves indisponibles) en N-1 = Compte 11
    $sql_reserves_n1 = "
        SELECT COALESCE(SUM(le.credit - le.debit), 0) as reserves
        FROM plan_comptable pc
        LEFT JOIN lignes_ecriture le ON pc.compte = le.compte AND le.societe_id = pc.societe_id
        ...
        AND pc.societe_id = ?
    ";
    $stmt_reserves_n1 = $db->prepare($sql_reserves_n1);
    $stmt_reserves_n1->execute([$date_fin_n1, $societe_id, $societe_id]);
    $passif_n1['CF']['net'] = $stmt_reserves_n1->fetchColumn();

    // CH (Report à nouveau) en N-1 = Compte 12 + Résultat N-2 (compte 13 jusqu'à fin N-2)
    $sql_report_12_n1 = "
        SELECT COALESCE(SUM(le.credit - le.debit), 0) as report_12
        FROM plan_comptable pc
        LEFT JOIN lignes_ecriture le ON pc.compte = le.compte AND le.societe_id = pc.societe_id
        ...
        AND pc.societe_id = ?
    ";
    $stmt_report_12_n1 = $db->prepare($sql_report_12_n1);
    $stmt_report_12_n1->execute([$date_fin_n1, $societe_id, $societe_id]);
    $report_12_n1 = $stmt_report_12_n1->fetchColumn();

    // Résultat N-2 (jusqu'à fin N-2)
    $sql_resultat_n2 = "
        SELECT COALESCE(SUM(le.credit - le.debit), 0) as resultat_n2
        FROM plan_comptable pc
        LEFT JOIN lignes_ecriture le ON pc.compte = le.compte AND le.societe_id = pc.societe_id
        LEFT JOIN ecritures e ON le.id_ecriture = e.id
        WHERE pc.actif = 'Oui'
        ...
        AND pc.societe_id = ?
    ";
    $stmt_resultat_n2 = $db->prepare($sql_resultat_n2);
    $stmt_resultat_n2->execute([$date_debut_n1, $societe_id, $societe_id]);
    $resultat_n2 = $stmt_resultat_n2->fetchColumn();
    ...
    $stmt_resultat_n1->execute([$date_fin_n1, $societe_id, $societe_id]);
...
<body class="bg-slate-900 text-slate-100">
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex-1">
                        ...
                        <p class="text-slate-400 mb-3" style="font-size: var(--font-size-base);">État de la situation patrimoniale du <?= date('d/m/Y', strtotime($date_debut)) ?> au <?= date('d/m/Y', strtotime($date_fin)) ?></p>

                        <?php
                        // Calculer pour le badge de santé financière
                        $cp_header = $passif['CP']['net'] ?? 0;
                        $capital_header = $passif['CA']['net'] ?? 0;
                        $total_actif_header = $actif['BZ']['net'] ?? 0;
                        $ratio_cp_header = $total_actif_header > 0 ? ($cp_header / $total_actif_header) * 100 : 0;
                        $ratio_cp_capital_header = $capital_header > 0 ? ($cp_header / $capital_header) * 100 : 0;

                        $sante_critique = $cp_header < 0;
                        $sante_art664 = !$sante_critique && $capital_header > 0 && $cp_header < ($capital_header / 2);
                        $sante_warning = !$sante_critique && !$sante_art664 && $ratio_cp_header < 10;
                        $sante_info = !$sante_critique && !$sante_art664 && !$sante_warning && $cp_header < ($passif_n1['CP']['net'] ?? 0);
                        ?>

                        <!-- Badge Santé Financière -->
                        <div class="flex items-center gap-2">
                            <span class="text-xs text-slate-400">Santé Financière:</span>
                            <?php if ($sante_critique): ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-red-500/20 text-red-400 text-xs font-semibold rounded-full border border-red-500/30">
                                    <span class="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
                                    Situation Critique
                                </span>
                                <span class="text-xs text-red-300">(CP: <?= number_format($cp_header, 0, ',', ' ') ?> FCFA)</span>
                            <?php elseif ($sante_art664): ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-orange-500/20 text-orange-400 text-xs font-semibold rounded-full border border-orange-500/30">
                                    <span class="w-2 h-2 bg-orange-500 rounded-full"></span>
                                    Article 664 - Action Requise
                                </span>
                                <span class="text-xs text-orange-300">(CP/Capital: <?= number_format($ratio_cp_capital_header, 1) ?>%)</span>
                            <?php elseif ($sante_warning): ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs font-semibold rounded-full border border-yellow-500/30">
                                    <span class="w-2 h-2 bg-yellow-500 rounded-full"></span>
                                    Attention Requise
                                </span>
                                <span class="text-xs text-yellow-300">(CP/Actif: <?= number_format($ratio_cp_header, 1) ?>%)</span>
                            <?php elseif ($sante_info): ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500/20 text-blue-400 text-xs font-semibold rounded-full border border-blue-500/30">
                                    <span class="w-2 h-2 bg-blue-500 rounded-full"></span>
                                    En Surveillance
                                </span>
                                <span class="text-xs text-blue-300">(CP en baisse)</span>
                            <?php else: ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/20 text-green-400 text-xs font-semibold rounded-full border border-green-500/30">
                                    <span class="w-2 h-2 bg-green-500 rounded-full"></span>
                                    Situation Normale
                                </span>
                                <span class="text-xs text-green-300">(CP: <?= number_format($cp_header, 0, ',', ' ') ?> FCFA)</span>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700 mb-6">
                <form method="GET">
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <!-- Bouton Excel avec comptes -->
                        <button type="button" onclick="exportExcelAvecComptes()" class="px-4 py-2 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2" title="Exporter avec le détail des comptes par rubrique">
                        ...
                        <?php if ($has_alerte): ?>
                            <?php if ($alerte_critique): ?>
                                <button type="button" onclick="openModal('modalCritique')" class="relative px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2 animate-pulse">
                            ...
                            <?php elseif ($alerte_art664): ?>
                                <button type="button" onclick="openModal('modalArticle664')" class="relative px-4 py-2 bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2">
                            ...
                            <?php elseif ($alerte_warning): ?>
                                <button type="button" onclick="openModal('modalWarning')" class="relative px-4 py-2 bg-gradient-to-r from-yellow-600 to-yellow-700 hover:from-yellow-700 hover:to-yellow-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2">
                            ...
                            <?php elseif ($alerte_info): ?>
                                <button type="button" onclick="openModal('modalInfo')" class="relative px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2">
            ...
            <!-- Modales d'Alertes (pas d'alertes visibles sur la page, seulement les modales) -->
            <?php if ($alerte_critique): ?>
            <!-- Modal Alerte Critique -->
            <div id="modalCritique" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 hidden items-center justify-center p-4" onclick="closeModal('modalCritique')">
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-red-500" onclick="event.stopPropagation()">
                    <!-- Header -->
                    <div class="sticky top-0 bg-gradient-to-r from-red-900 to-red-800 p-6 border-b border-red-700 flex items-center justify-between">
                        ...
                        <button onclick="closeModal('modalCritique')" class="w-10 h-10 rounded-full hover:bg-red-700 flex items-center justify-center transition-colors">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-red-900/30 border border-red-500 rounded-xl p-5">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <p class="text-sm text-red-200">Capitaux Propres (N):</p>
                                    <p class="text-2xl font-bold text-red-400"><?= number_format($cp_n, 2, ',', ' ') ?> FCFA</p>
                                </div>
                                <div>
                                    <p class="text-sm text-red-200">Capital Social:</p>
                                    <p class="text-xl font-bold text-white"><?= number_format($capital_social, 2, ',', ' ') ?> FCFA</p>
                                </div>
                            </div>
                            <p class="mt-4 text-red-100">
                                <i class="fas fa-info-circle mr-2"></i>
                                Votre entreprise est en <strong>situation de capitaux propres négatifs</strong>, ce qui signifie que
                                les dettes excèdent les actifs nets. Cette situation requiert une attention immédiate.
                            </p>
                        </div>

                        <!-- Article 664 AUDSCGIE -->
                        <div class="bg-red-800/40 border border-red-600 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-red-100 mb-3 flex items-center gap-2">
                                <i class="fas fa-gavel"></i> Article 664 AUDSCGIE
                            </h3>
                            <p class="text-sm italic text-red-100 leading-relaxed mb-3 border-l-4 border-red-500 pl-4">
                        ...
                        <div class="bg-slate-800/50 border border-slate-600 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-orange-300 mb-3 flex items-center gap-2">
                                <i class="fas fa-clipboard-list"></i> Obligations Légales (Article 664)
                            </h3>
                            <ul class="space-y-2 text-sm text-slate-200">
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-check-circle text-orange-400 mt-1"></i>
                                    <span><strong>Délai:</strong> Convoquer l'AGE dans les 4 mois suivant l'approbation des comptes</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-check-circle text-orange-400 mt-1"></i>
                                    <span><strong>Décision:</strong> L'AGE doit statuer sur la dissolution anticipée ou la poursuite d'activité</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-check-circle text-orange-400 mt-1"></i>
                                    <span><strong>Mesures:</strong> En cas de poursuite, décider des mesures de redressement (augmentation de capital, apports, etc.)</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-check-circle text-orange-400 mt-1"></i>
                                    <span><strong>Responsabilité:</strong> Les dirigeants engagent leur responsabilité en cas de non-respect</span>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="bg-slate-800/50 border border-slate-600 rounded-xl p-5">
                            ...
                            <ul class="space-y-2 text-sm text-slate-200">
                                ...
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-arrow-right text-cyan-400 mt-1"></i>
                                    <span>Préparer un plan de redressement (augmentation de capital, apports en compte courant, cession d'actifs)</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-arrow-right text-cyan-400 mt-1"></i>
                                    <span>Consulter un expert-comptable et un avocat spécialisé en droit OHADA</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-arrow-right text-cyan-400 mt-1"></i>
                                    <span>Documenter toutes les décisions prises pour justifier la bonne foi des dirigeants</span>
                                </li>
                                <li class="flex items-start gap-2">
                                    <i class="fas fa-arrow-right text-cyan-400 mt-1"></i>
                                    <span>Consulter le rapport d'évolution pour comprendre la progression des pertes</span>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="flex justify-between pt-4">
                            <a href="../rapports/evolution_report_nouveau.php"
                               class="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg font-semibold transition-all shadow-lg hover:shadow-xl inline-flex items-center gap-2">
            ...
            <?php elseif ($alerte_art664): ?>
            <!-- Modal Article 664 -->
            <div id="modalArticle664" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 hidden items-center justify-center p-4" onclick="closeModal('modalArticle664')">
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-orange-500" onclick="event.stopPropagation()">
                    <!-- Header -->
                    <div class="sticky top-0 bg-gradient-to-r from-orange-900 to-orange-800 p-6 border-b border-orange-700 flex items-center justify-between">
                        <div class="flex items-center gap-4">
                        ...
                        <button onclick="closeModal('modalArticle664')" class="w-10 h-10 rounded-full hover:bg-orange-700 flex items-center justify-center transition-colors">
                            <i class="fas fa-times text-white text-xl"></i>
                        </button>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-orange-900/30 border border-orange-500 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-orange-300 mb-4 flex items-center gap-2">
                                <i class="fas fa-chart-pie"></i> Situation Financière
                            </h3>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div>
                                    <p class="text-xs text-orange-300 mb-1">Capital Social (CA)</p>
                                    <p class="text-lg font-bold text-white"><?= number_format($capital_social, 2, ',', ' ') ?></p>
                                    <p class="text-xs text-orange-200">FCFA</p>
                                </div>
                                <div>
                                    <p class="text-xs text-orange-300 mb-1">Capitaux Propres (CP)</p>
                                    <p class="text-lg font-bold text-orange-400"><?= number_format($cp_n, 2, ',', ' ') ?></p>
                                    <p class="text-xs text-orange-200">FCFA</p>
                                </div>
                                <div>
                                    <p class="text-xs text-orange-300 mb-1">Seuil de 50%</p>
                                    <p class="text-lg font-bold text-yellow-400"><?= number_format($capital_social / 2, 2, ',', ' ') ?></p>
                                    <p class="text-xs text-orange-200">FCFA</p>
                                </div>
                                <div>
                                    <p class="text-xs text-orange-300 mb-1">Ratio CP / Capital</p>
                                    <p class="text-lg font-bold <?= $ratio_cp_capital < 0 ? 'text-red-400' : 'text-orange-400' ?>">
                                        <?= number_format($ratio_cp_capital, 2, ',', ' ') ?>%
                                    </p>
                                    <p class="text-xs text-orange-200">< 50% requis</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-orange-800/40 border border-orange-600 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-orange-100 mb-3 flex items-center gap-2">
                                <i class="fas fa-gavel"></i> Texte de l'Article 664 AUDSCGIE
                            </h3>
                            <div class="bg-orange-900/50 border-l-4 border-orange-500 p-4 rounded">
                                <p class="text-xs text-orange-300 mb-2 font-semibold">
                                    Acte Uniforme OHADA relatif au Droit des Sociétés Commerciales et du Groupement d'Intérêt Économique
                                </p>
                                <p class="text-sm italic text-orange-100 leading-relaxed">
                                    "Si, du fait de pertes constatées dans les états financiers de synthèse, les <strong>capitaux propres de la société
                                    deviennent inférieurs à la moitié du capital social</strong>, le conseil d'administration ou l'administrateur général,
                                    selon le cas, est tenu, <strong>dans les quatre mois qui suivent l'approbation des comptes</strong> ayant fait apparaître
                                    cette perte, de <strong>convoquer l'assemblée générale extraordinaire</strong> à l'effet de décider s'il y a lieu
                                    à <strong>dissolution anticipée de la société</strong>."
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-800/50 border border-slate-600 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-yellow-300 mb-3 flex items-center gap-2">
                                <i class="fas fa-clipboard-list"></i> Obligations Légales
                            </h3>
                            <ol class="space-y-3">
                                <li class="flex items-start gap-3">
                                    <div class="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                                        1
                                    </div>
                                    <div class="text-sm text-slate-200">
                                        <strong class="text-white">Délai de 4 mois:</strong> À compter de l'approbation des comptes par l'Assemblée Générale Ordinaire
                                    </div>
                                </li>
                                <li class="flex items-start gap-3">
                                    <div class="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                                    ...
                                    <div class="text-sm text-slate-200">
                                        <strong class="text-white">Convocation AGE:</strong> Assemblée Générale Extraordinaire obligatoire (par le conseil d'administration ou l'administrateur général)
                                    </div>
                                </li>
                                <li class="flex items-start gap-3">
                                    <div class="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                                    ...
                                    <div class="text-sm text-slate-200">
                                        <strong class="text-white">Ordre du jour:</strong> Décision sur la dissolution anticipée de la société ou la poursuite d'activité
                                    </div>
                                </li>
                                <li class="flex items-start gap-3">
                                    <div class="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                                    ...
                                    <div class="text-sm text-slate-200">
                                        <strong class="text-white">Si poursuite d'activité:</strong> L'AGE doit décider des mesures concrètes de reconstitution des capitaux propres
                                    </div>
                                </li>
                                <li class="flex items-start gap-3">
                                    <div class="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                                    ...
                                    <div class="text-sm text-slate-200">
                                        <strong class="text-white">Délai de reconstitution:</strong> Généralement dans un délai raisonnable (à préciser dans la décision de l'AGE)
                                    </div>
                                </li>
                            </ol>
                        </div>
                        ...
                        <div class="bg-red-900/20 border border-red-700 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-red-300 mb-3 flex items-center gap-2">
                                <i class="fas fa-exclamation-triangle"></i> Sanctions en Cas de Non-Respect
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div class="bg-red-900/30 p-3 rounded-lg">
                                    ...
                                    <p class="text-xs text-red-100">Des dirigeants pour faute de gestion</p>
                                </div>
                                <div class="bg-red-900/30 p-3 rounded-lg">
                                    <p class="text-sm font-semibold text-red-200 mb-1">
                                        <i class="fas fa-handcuffs text-red-400 mr-2"></i>Responsabilité Pénale
                                    </p>
                                    <p class="text-xs text-red-100">Sanctions selon les législations nationales</p>
                                </div>
                                <div class="bg-red-900/30 p-3 rounded-lg">
                                    <p class="text-sm font-semibold text-red-200 mb-1">
                                        <i class="fas fa-ban text-red-400 mr-2"></i>Nullité des Actes
                                    </p>
                                    <p class="text-xs text-red-100">Décisions prises après le délai annulables</p>
                                </div>
                                <div class="bg-red-900/30 p-3 rounded-lg">
                                    <p class="text-sm font-semibold text-red-200 mb-1">
                                        <i class="fas fa-building text-red-400 mr-2"></i>Dissolution Judiciaire
                                    </p>
                                    <p class="text-xs text-red-100">Tout intéressé peut demander au tribunal</p>
                                </div>
                            </div>
                        </div>

                        <!-- Calculateur de Reconstitution des CP -->
                        <div class="bg-gradient-to-br from-cyan-900/20 to-blue-900/20 border border-cyan-700 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-cyan-300 mb-3 flex items-center gap-2">
                                <i class="fas fa-calculator"></i> Calculateur de Reconstitution
                            </h3>

                            <div class="space-y-4">
                                <!-- Situation Actuelle -->
                                <div class="bg-slate-800/50 p-3 rounded-lg">
                                    <p class="text-xs text-slate-400 mb-2">Situation Actuelle</p>
                                    <div class="grid grid-cols-2 gap-3 text-sm">
                                        <div>
                                            <span class="text-slate-300">CP Actuels:</span>
                                            <span class="font-mono font-bold text-orange-400 ml-2"><?= number_format($cp_n, 0, ',', ' ') ?></span>
                                        </div>
                                        <div>
                                            <span class="text-slate-300">Capital Social:</span>
                                            <span class="font-mono font-bold text-white ml-2"><?= number_format($capital_social, 0, ',', ' ') ?></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                $deficit = $seuil_50_pct - $cp_n;
                                $pour_60_pct = ($capital_social * 0.6) - $cp_n;
                                $pour_75_pct = ($capital_social * 0.75) - $cp_n;
                                $pour_100_pct = $capital_social - $cp_n;
                                ?>

                                <!-- Scénarios de Reconstitution -->
                                <div class="space-y-2">
                                    <p class="text-xs font-semibold text-cyan-300 mb-2">Montants Nécessaires:</p>

                                    <!-- Scénario 1: Atteindre 50% (Minimum Légal) -->
                                    <div class="flex items-center justify-between bg-orange-900/20 border border-orange-700/30 p-2 rounded">
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-arrow-up text-orange-400 text-xs"></i>
                                            <span class="text-xs text-orange-300">Atteindre 50% (Minimum Légal)</span>
                                        </div>
                                        <span class="font-mono font-bold text-orange-400 text-sm"><?= number_format($deficit, 0, ',', ' ') ?> FCFA</span>
                                    </div>

                                    <!-- Scénario 2: Atteindre 60% (Sécurisé) -->
                                    <div class="flex items-center justify-between bg-yellow-900/20 border border-yellow-700/30 p-2 rounded">
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-arrow-up text-yellow-400 text-xs"></i>
                                            <span class="text-xs text-yellow-300">Atteindre 60% (Sécurisé)</span>
                                        </div>
                                        <span class="font-mono font-bold text-yellow-400 text-sm"><?= number_format($pour_60_pct, 0, ',', ' ') ?> FCFA</span>
                                    </div>

                                    <!-- Scénario 3: Atteindre 75% (Recommandé) -->
                                    <div class="flex items-center justify-between bg-cyan-900/20 border border-cyan-700/30 p-2 rounded">
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-arrow-up text-cyan-400 text-xs"></i>
                                            <span class="text-xs text-cyan-300">Atteindre 75% (Recommandé)</span>
                                        </div>
                                        <span class="font-mono font-bold text-cyan-400 text-sm"><?= number_format($pour_75_pct, 0, ',', ' ') ?> FCFA</span>
                                    </div>

                                    <!-- Scénario 4: Atteindre 100% (Optimal) -->
                                    <div class="flex items-center justify-between bg-green-900/20 border border-green-700/30 p-2 rounded">
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-arrow-up text-green-400 text-xs"></i>
                                            <span class="text-xs text-green-300">Atteindre 100% (Optimal)</span>
                                        </div>
                                        <span class="font-mono font-bold text-green-400 text-sm"><?= number_format($pour_100_pct, 0, ',', ' ') ?> FCFA</span>
                                    </div>
                                </div>

                                <!-- Options de Reconstitution -->
                                <div class="bg-slate-800/50 p-3 rounded-lg">
                                    ...
                                    <ul class="space-y-1 text-xs text-slate-300">
                                        <li class="flex items-start gap-1">
                                            <i class="fas fa-chevron-right text-cyan-400 mt-0.5 text-[10px]"></i>
                                            <span><strong>Augmentation de capital:</strong> Apport en numéraire ou en nature</span>
                                        </li>
                                        <li class="flex items-start gap-1">
                                            <i class="fas fa-chevron-right text-cyan-400 mt-0.5 text-[10px]"></i>
                                            <span><strong>Incorporation de réserves:</strong> Si disponibles</span>
                                        </li>
                                        <li class="flex items-start gap-1">
                                            <i class="fas fa-chevron-right text-cyan-400 mt-0.5 text-[10px]"></i>
                                            <span><strong>Abandon de compte courant:</strong> Par les associés</span>
                                        </li>
                                        <li class="flex items-start gap-1">
                                            <i class="fas fa-chevron-right text-cyan-400 mt-0.5 text-[10px]"></i>
                                            <span><strong>Conversion de dettes:</strong> En capital social</span>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div class="bg-orange-900/20 border-l-4 border-orange-500 p-3 rounded">
                                    <p class="text-xs text-orange-200">
                                        ...
                                        <strong>Important:</strong> Ces calculs sont indicatifs. Consultez un expert-comptable pour élaborer un plan de redressement adapté à votre situation.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-800/50 border border-slate-600 rounded-xl p-5">
                            <h3 class="text-lg font-bold text-cyan-300 mb-3 flex items-center gap-2">
                                <i class="fas fa-tasks"></i> Actions Recommandées
                            </h3>
                            <div class="space-y-2">
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Préparer la convocation de l'Assemblée Générale Extraordinaire dans les 4 mois</span>
                                </div>
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Préparer un rapport de gestion détaillé expliquant la situation et les causes</span>
                                </div>
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Proposer des solutions: augmentation de capital, apports en compte courant, incorporation de réserves</span>
                                </div>
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Consulter un expert-comptable et un conseil juridique spécialisé en droit OHADA</span>
                                </div>
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Documenter toutes les démarches entreprises pour justifier la bonne foi</span>
                                </div>
                                <div class="flex items-start gap-2 text-sm text-slate-200">
                                    <i class="fas fa-check text-cyan-400 mt-1"></i>
                                    <span>Analyser l'évolution du report à nouveau pour comprendre la tendance</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between pt-4">
                            ...
                            <button onclick="closeModal('modalArticle664')"
                                    class="px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-semibold transition-colors">
                                Fermer
                            </button>
                        </div>
            ...
            <?php elseif ($alerte_warning): ?>
            <!-- Alerte Warning: Capitaux Propres Faibles -->
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-yellow-300 mb-2">
                            ⚠️ Attention: Capitaux Propres Faibles
                        </h3>
                        <div class="text-yellow-200 space-y-2">
                            <p>
                                Capitaux Propres (N): <strong><?= number_format($cp_n, 2, ',', ' ') ?> FCFA</strong>
                                (<?= number_format($ratio_cp, 2, ',', ' ') ?>% de l'actif total)
                            </p>
                            <p>
                                Les capitaux propres représentent moins de 10% de l'actif total.
                                Une amélioration de la rentabilité ou un apport de fonds est recommandé.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <?php elseif ($alerte_info): ?>
            <!-- Alerte Info: CP en Diminution -->
            <div class="mb-6 bg-blue-900/30 border border-blue-500 rounded-xl p-4">
                <div class="flex items-start gap-3">
                    <i class="fas fa-info-circle text-blue-400 text-xl mt-1"></i>
                    <div class="flex-1">
                        <h4 class="font-semibold text-blue-300 mb-1">Information: Capitaux Propres en Diminution</h4>
                        <p class="text-blue-200 text-sm">
                            CP (N): <?= number_format($cp_n, 2, ',', ' ') ?> FCFA |
                            CP (N-1): <?= number_format($cp_n1, 2, ',', ' ') ?> FCFA |
                            Variation: <?= number_format($cp_n - $cp_n1, 2, ',', ' ') ?> FCFA
                        </p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function exportExcel() {
            const params = new URLSearchParams({
                date_debut: '<?= $date_debut ?>',
                date_fin: '<?= $date_fin ?>',
                mode: isCondensed ? 'condense' : 'detail'
            });
            window.location.href = 'export_bilan_excel.php?' + params.toString();
        }

        function exportExcelAvecComptes() {
            const params = new URLSearchParams({
                date_debut: '<?= $date_debut ?>',
                date_fin: '<?= $date_fin ?>',
                mode: 'comptes'
    ...
    <script>
        // Filtre balance : n'afficher que les comptes de Bilan (classes 1 à 5)
        window.balanceTableauFilter = 'Bilan';
    </script>
